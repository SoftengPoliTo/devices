...
//use ascot_esp32c3::devices::camera::Camera;
...
fn main() -> ascot_esp32c3::error::Result<()> {
    ...
    // If you use `tokio`, you still have to do the same as `tokio` also
    // uses the `eventfd` syscall
    ...
    let stream_action = DeviceAction::new(
        // Configuration for the `GET` stream route.
        Route::get("/stream").description("Retrieve stream URL."),
        // TODO: Implement a response for temperature
        ResponseBuilder(|req| req.into_ok_response(), "Stream URL detected!"),
    )
    .body(move || {
        // TODO: Retrieve stream URL

        Ok(())
    });

    // TODO: Implement camera
    //let camera = Camera::new(stream_action).build();
